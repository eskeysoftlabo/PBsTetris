...
test('a protocol id already in use is named as such',function()
 local taken=stubLibrary("Protocol with ID 510 already exists with name 'SomeOtherAddon'.")
 assert(taken.error:find('二重に導入'),'the report points at a duplicate install: '..taken.error)
 assert(taken.error:find('SomeOtherAddon'),'and names what is holding the id: '..taken.error)
